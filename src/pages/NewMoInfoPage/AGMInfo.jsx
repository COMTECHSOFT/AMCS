...
const AGMInfo = () => {
  ...
  const [agmCode, setAgmCode] = useState(0);
  const [datas, setDatas] = useState([]);
  const moData = datas?.find((data) => data);
  useEffect(() => {
    fetch(`http://192.168.31.94/api/agm_info.php?CODE=${agmCode}`) // 000099
      .then((res) => res.json())
      .then((data) => setDatas(data.agm_info));
  }, [agmCode]);
  ...
  const handleSave = () => {
    ...
    if (!agmCode) {
      ...
      const url = "http://192.168.31.94/api/agm_insert.php";
      fetch(url, {
        method: "POST",
        mode: "no-cors",
        headers: {
        ...
        body: JSON.stringify(agmSaveData),
      })
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
