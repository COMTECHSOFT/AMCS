...
const DGMInfo = () => {
  ...
  useEffect(() => {
    fetch(`http://192.168.31.94/api/dir_info.php?CODE=${dirCode}`) // 000099
      .then((res) => res.json())
      .then((data) => setDatas(data.dir_info));
  }, [dirCode]);
  ...
  const handleSave = () => {
    ...
    if (!dirCode) {
      ...
      const url = "http://192.168.31.94/api/dir_insert.php";
      fetch(url, {
        method: "POST",
        mode: "no-cors",
        headers: {
        ...
        body: JSON.stringify(dirSaveData),
      })
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
