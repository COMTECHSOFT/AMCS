...
const DGMInfo = () => {
  ...
  const [edCode, setEdCode] = useState(0);
  const [datas, setDatas] = useState([]);
  const moData = datas?.find((data) => data);
  useEffect(() => {
    fetch(`http://192.168.31.94/api/ed_info.php?CODE=${edCode}`) // 000099
      .then((res) => res.json())
      .then((data) => setDatas(data.ed_info));
  }, [edCode]);
  ...
  const handleSave = () => {
    ...
    if (!edCode) {
      ...
      const url = "http://192.168.31.94/api/ed_insert.php";
      fetch(url, {
        method: "POST",
        mode: "no-cors",
        headers: {
        ...
        body: JSON.stringify(edSaveData),
      })
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
